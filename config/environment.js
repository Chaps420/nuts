...
(function() {
    ...
    // Always use production Firebase Functions when not on localhost
    const FIREBASE_FUNCTIONS_URL = isDevelopment
        ? `http://localhost:5001/${FIREBASE_PROJECT_ID}/${FIREBASE_REGION}` // Emulator URL
        : `https://${FIREBASE_REGION}-${FIREBASE_PROJECT_ID}.cloudfunctions.net`; // Production
    ...
    // Always use production endpoints when not on localhost
    if (!isDevelopment) {
        const prodUrl = `https://${FIREBASE_REGION}-${FIREBASE_PROJECT_ID}.cloudfunctions.net`;
        endpoints.createNutsPayment = `${prodUrl}/createNutsPayment`;
        endpoints.payloadStatus = `${prodUrl}/payloadStatus`;
        endpoints.baseUrl = prodUrl;
    }
