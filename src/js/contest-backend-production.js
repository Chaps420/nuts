...
class ContestBackendProduction {
     ...
     */
    async updateEntryScore(entryId, score, contestDate) {
        try {
            const response = await fetch(`${this.baseUrl}/updateEntryScore`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            const result = await response.json();
            console.log(`📊 Updated entry ${entryId} score to ${score}:`, result);
            return result;
        } catch (error) {
            console.error(`❌ Failed to update entry ${entryId} score:`, error);
            throw error;
        }
    }
