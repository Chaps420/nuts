...
class NutsApp {
  ...
  async waitForDependencies() {
    ...
    if (window.WalletManager) {
      ...
      console.warn('WalletManager not available, continuing without wallet functionality');
    }
    
    // Initialize other managers (these might not exist yet)
    this.contestManager = {
      init: async () => {},
      getActiveContests: async () => [],
      getLatestWinners: async () => []
    };
  ...
  async initializeComponents() {
    // Initialize wallet manager if available
    if (this.walletManager) {
      await this.walletManager.init();
    }
    
    // Initialize contest manager
    if (this.contestManager && this.contestManager.init) {
      await this.contestManager.init();
    }
    
    // Initialize odds API
    if (this.oddsAPI && this.oddsAPI.init) {
    ...
    connectButtons.forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (this.walletManager) {
          ...
          this.showError('Wallet functionality not available');
        }
      });
    });
  ...
  async loadActiveContests() {
    try {
      if (this.contestManager && this.contestManager.getActiveContests) {
  ...
  async loadLatestWinners() {
    try {
      if (this.contestManager && this.contestManager.getLatestWinners) {
  ...
  async loadUpcomingGames() {
    try {
      if (this.oddsAPI && this.oddsAPI.getUpcomingGames) {
  ...
  updateWinnersDisplay(winners) {
    ...
    if (!winnersContainer || !winners || !winners.length) return;
  ...
  updateGamesPreview(games) {
    ...
    if (!gamesContainer || !games || !games.length) return;
  ...
  onWalletConnected(walletData) {
    ...
    if (walletInfo) {
      walletInfo.innerHTML = `
        ...
        <button class="disconnect-wallet-btn" onclick="window.walletManager && window.walletManager.disconnectWallet()">
          Disconnect
        </button>
      `;
    ...
    if (this.walletManager && this.walletManager.updateWalletUI) {
...
// Make NutsApp available globally
window.NutsApp = NutsApp;

// Initialize app when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
  console.log('🌐 DOM loaded, initializing $NUTS app...');
